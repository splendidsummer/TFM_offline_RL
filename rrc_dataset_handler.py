...
def symmetry_push_aug_cw120(raw_dataset, save_path):
    ...
    with tqdm(total=raw_dataset['observations'].shape[0], desc="Augmenting the push task cw120") as pbar:
        ...
        for idx, obs in enumerate(raw_dataset['observations']):
            ...
            # orientation and position
            ag_key_points = copy(temp_obs[17:17+24]).reshape(8, 3)  # [17:41]
            ag_pos, ag_ori = rrc_2022_datasets.utils.get_pose_from_keypoints(ag_key_points)[
                0], rrc_2022_datasets.utils.get_pose_from_keypoints(ag_key_points)[1]
