...
class MeanQFunctionFactory(QFunctionFactory):
    ...
    def create_equivaraint_discrete(
        self,
        encoder: Encoder,  # here we import an end-to-end
        hidden_size: int,
        action_size: int,
    ):  # Original Output Type: -> Tuple[DiscreteMeanQFunction, DiscreteMeanQFunctionForwarder]
        q_func = EquivariantDiscreteMeanQFunction(encoder, hidden_size, action_size)
        forwarder = DiscreteMeanQFunctionForwarder(q_func, action_size)
        return q_func, forwarder
    ...
    def create_equivariant_continuous(
        ...
        q_func = EquivariantContinuousMeanQFunction(encoder, hidden_size)
        forwarder = ContinuousMeanQFunctionForwarder(q_func)
        return q_func, forwarder
