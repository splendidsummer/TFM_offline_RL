...
from ...models.builders import (
    ...
    create_equivariant_discrete_q_function,
...
class DiscreteCQLConfig(LearnableConfig):
    ...
    Args:
        observation_scaler (d3rlpy.preprocessing.ObservationScaler):
            Observation preprocessor.                                           (from LearnableConfig)
        reward_scaler (d3rlpy.preprocessing.RewardScaler): Reward preprocessor. (from  LearnableConfig)
        ...
        escnn: whether we build the equivariant NN or not.
    """
...
class DiscreteCQL(QLearningAlgoBase[DiscreteCQLImpl, DiscreteCQLConfig]):
    def inner_create_impl(
        ...
        if self._config.escnn:
            q_funcs, q_func_forwarder = create_equivariant_discrete_q_function(
                 observation_shape,
                 action_size,
            ...
            targ_q_funcs, targ_q_func_forwarder = create_equivariant_discrete_q_function(
                observation_shape,
                action_size,
                self._config.encoder_factory,
                # C2InvEncoderFactory(),
